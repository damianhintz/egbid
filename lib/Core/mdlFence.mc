...
int mdlFence_deleteFunc(void* argP) {
    ULong filePos;
    NumerPlikuDgn modelRef;

    filePos = mdlElement_getFilePos(FILEPOS_CURRENT, &modelRef);

    if (filePos != 0xffffffff) {
        if (SUCCESS != mdlElmdscr_undoableDelete(NULL, filePos, FALSE))
            mdlUtil_dolaczBlad("mdlFence_deleteFunc", "undoable delete failure");
    } else
        mdlUtil_dolaczBlad("mdlFence_deleteFunc", "element getFilePos failure");
