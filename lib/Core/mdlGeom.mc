...
/* mdlGeom_zawieraPunkt - czy punkt jest wewn¹trz obszaru punktów */
...
DPoint3d* mdlGeom_pobierzPunkty(MSElementDescr* edP, NumerPlikuDgn modelRefP, int* nPunktyP) {
...
DPoint3d* mdlGeom_pobierzPunktyKompleksu(MSElementDescr* edP, NumerPlikuDgn modelRefP, int* nPunktyP) {
    char msg[128];
    ...
    *nPunktyP = 0;
    ...
    while (pComponent) {
        ...
        if (obiektDgn_jestProsty(elemType)) {
            ...
            sprintf(msg, "element nie jest liniowy (%d)", elemType);
            mdlUtil_dolaczBlad("mdlGeom_pobierzPunktyKompleksu", msg);
        }
...
int mdlGeom_obliczPunktyKompleksu(MSElementDescr* edP, NumerPlikuDgn modelRefP, int* nPunktyP) {
    char msg[128];
    DPoint3d* aPunktyTemp = NULL;
    int nPunktyTemp = 0;
    MSElementDescr* pComponent = NULL;
    ...
    *nPunktyP = 0;
    ...
    while (pComponent) {
        ...
        if (obiektDgn_jestProsty(elemType)) {
            ...
            sprintf(msg, "element nie jest liniowy %d", elemType);
            mdlUtil_dolaczBlad("mdlGeom_obliczPunktyKompleksu", msg);
        }
...
DPoint3d* mdlGeom_pobierzPunktyWszystkie(MSElementDescr* edP, NumerPlikuDgn modelRefP, int* nPunktyP) {
...
int scan_checkElement(DPoint3d* pts, int numpts, int* a, int* b) {
    ...
    if (pts != NULL && numpts > 3) {
        ...
        /* pierwsze za³amanie mo¿e byæ na samym pocz¹tku, miêdzy n;0;1 */
        p1 = &pts[numpts - 1];
        p2 = &pts[0];
        p3 = &pts[1];
        ...
        /* sprawdzanie za³amañ 0;1 1;2 n-1;n */
        for (i = 1; i < nZal; i++) {
            lensum = 0.0;
        ...
        /* teraz trzeba sprawdziæ jeszcze za³amanie miêdzy n i 0 */
        /* odleg³oœæ od za³amania 0 do punktu 0 */
        lensum = 0.0;
        for (j = pZal[nZal - 1] + 1; j < numpts; j++) {
            lensum += mdlVec_distance(&pts[j - 1], &pts[j]);
        }
        /* odleg³oœæ od za³amania n do punktu 0 */
        for (j = 1; j <= pZal[0]; j++) {
            lensum += mdlVec_distance(&pts[j - 1], &pts[j]);
        }
...
double mdlGeom_Intersect(MSElementDescr* edPewid, MSElementDescr* edPbud, NumerPlikuDgn modelRefP) {
    ...
    /* budynek powinien nie mieæ w œrodku komórki adres */
    ...
    if (SUCCESS == mdlMeasure_polygonArea(&f_budPole, &perimeter, a_budPunkty, n_budPunkty)) {
        ...
        if (SUCCESS == (status = mdlElmdscr_intersectShapes(&edPint, NULL, edPbud, edPewid, tolerancja))) {
            ...
            if (SUCCESS == mdlMeasure_polygonArea(&f_intPole, &perimeter, a_intPunkty, n_intPunkty)) {
                ...
                nachodzenie = -1.0;
                mdlUtil_dolaczBlad("mdlGeom_Intersect", "nie mo¿na obliczyæ pola nachodzenia");
            }
            mdlElmdscr_freeAll(&edPint);
        } else {
            if (status == MDLERR_NULLSOLUTION) {
                nachodzenie = 0.0;
                //util_nfoPrint ("geo_Intersect: puste przeciêcie");
            } else {
                nachodzenie = -1.0;
                //util_errPrint ("geo_Intersect: b³¹d przeciêcia");
            }
        }
    } else {
        nachodzenie = -1.0;
        mdlUtil_dolaczBlad("mdlGeom_Intersect", "nie mo¿na obliczyæ pola budynku");
    }
...
/* oblicz k¹t w stopniach miêdzy trzema punktami */
