...
int mdlElem_cellLine(MSElementDescr* edP, NumerPlikuDgn modelRefP, MSElementUnion* elLineP, DPoint3d* shape, RotMatrix* matrixP) {
    ...
    if (SUCCESS == mdlLine_create(elLineP, &edP->el, psLine)) {
        ...
        mdlUtil_dolaczBlad("mdlElem_cellLine", "nie mo¿na utworzyæ linii");
    ...
    while (pComponent) {
        ...
        if (ELLIPSE_ELM == elemType) {
            if (SUCCESS != mdlArc_extract(startEndPts, &start, &sweep, &axis1, &axis2, &rotMatrix, &center, el)) {
                mdlUtil_dolaczBlad("dgnCell_extractEllipse", "nie mo¿na odczytaæ parametrów elipsy");
                return FALSE;
            }
...
int dgnCell_extractRect(MSElementDescr* edP, NumerPlikuDgn modelRefP, MSElementUnion* prostokat, double scale, int* visible) {
    ...
    while (pComponent) {
        ...
        if (LINE_ELM == elemType) {
            ...
            if (NULL == (aPtsP = mdlGeom_pobierzPunkty(pComponent, modelRefP, &nPts))) {
                mdlUtil_dolaczBlad("dgnCell_extractRect", "nie mo¿na odczytaæ parametrów linii");
                return FALSE;
            }
    ...
    if (bKwadrat) {
        ...
        if (SUCCESS != mdlShape_create(prostokat, NULL, aKwadrat, 4, 0)) {
            mdlUtil_dolaczBlad("dgnCell_extractRect", "nie mo¿na utworzyæ prostokata");
            bKwadrat = FALSE;
        }
...
int obiektDgn_rysujPunktyNaWidoku(DPoint3d* aPunkty, int nPunkty, byte bParametry) {
    ...
    for (i = 0; i < nPunkty; i++) {
        ...
        if (bKropki) {
            ...
            //oblicz wektor prostej dwóch kolejnych punktów i go znormalizuj
            ...
            //obliczenie wektorów prostopad³ych do wektora prostej
            vecA.x = vec1.y;
            vecA.y = -vec1.x;
            vecA.z = 0.0;
            vecB.x = -vec1.y;
            ...
            //skalowanie obliczonych wektorów prostopad³ych
            ...
            //obliczanie punktów prostopad³ych do punktu na prostej
            mdlVec_addPoint(&pA, &p12, &vecA);
            mdlVec_addPoint(&pB, &p12, &vecB);

            //konwertujemy obliczone punktu do uk³adu lokalnego widoku
            //zamiana wspolrzednych punktu (IN UORS) na wspolrzedne widoku (x w prawo, y w dó³)
            //wzgledem okna widoku (lewy, górny naro¿nik)
